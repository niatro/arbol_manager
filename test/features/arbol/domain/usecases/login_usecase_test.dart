...
void main() {
  ...
  test(
      ...
      () async {
    // arrange
    when(mockArbolesRepositorio.login(password: anyNamed("password")))
        .thenAnswer((_) async => Right(usuarioTest));
    // act
    final result = await usecase.call(Params(password: password));
    // assert
    expect(result, Right(usuarioTest));
    verify(mockArbolesRepositorio.login(password: password));
